...
\node[annotation, centered, font=, yshift = -6.5cm, text width = 6.5cm] at (non-lin)
	...
	given $x + f'(p) \neq 0$
	$$\frac{dy}{dx} = x\frac{dp}{dx} + p + \frac{df}{dp} \frac{dp}{dx}$$
	$$0 = \frac{dp}{dx} (x + f'(p))$$
	since $$x + f'(p) \neq 0, \frac{dp}{dx} = 0$$
...
\node[annotation, centered, font=, yshift = -5cm, xshift = -5.5cm, text width = 8.5cm] at (2-homo)
...
\node[annotation, centered, font=, xshift = 8cm, yshift = -12cm, text width = 11.5cm] at (LT)
	...
	$$\lim\limits_{M \rightarrow \infty}  \left[e^{-st} Y(t) |^M_0 + s\int_{0}^{M} e^{-st} Y(t) dt\right]$$
	$$\lim\limits_{M \rightarrow \infty} \left[e^{-sM} Y(M) -  Y(0) + s\int_{0}^M e^{-st} Y(t) dt\right]$$
	$$sy(s) - Y(0), \hspace{1cm} y(s) = L\{Y(t)\}$$
