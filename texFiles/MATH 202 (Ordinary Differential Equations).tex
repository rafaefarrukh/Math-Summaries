...
\node [scale = 2] {\textbf{Ordinary Differential Equations \\ (ODE)}}
	child [sibling angle = 180] {node [scale = 1.5] {\textbf{First Order}}
		child [sibling angle = 60] {node (lin) {\textbf{Linear}}
			child [sibling angle = 75] {node (non-lin) {\textbf{Clairaut}}}
	...
	child [sibling angle = 180] {node [scale = 1.5] {\textbf{Linear \\ Second and Higher Orders}}
		...
		child [sibling angle = 0] {node {\textbf{Constant Coefficient}}
			child [sibling angle = 180] {node (2-homo) {\textbf{Homogenous}}
				...
				child [sibling angle = 70] {node (repeat) {\textbf{Repeat Roots} $$m_1 = m_2$$}}
...
\node[annotation, centered, font=, yshift = -6.5cm, text width = 6.5cm] at (red-lin)
	...
	multiply by $y^{1-n}$
	$$\frac{1}{y^n} (\frac{dy}{dx}) + P(x)y^{1-n} = Q(x) \hspace{0.1cm} -(1)$$
	use sub: $v = y^{1-n}$
	$$\frac{dv}{dx} = (1-n)y^{-n} \frac{dy}{dx}$$
	$$\frac{1}{y^n} (\frac{dy}{dx}) = \frac{dv}{dx} (\frac{1}{1-n}) \hspace{0.1cm} -(2)$$
	sub (2) in (1)
...
\node[annotation, centered, font=, yshift = -6.5cm, text width = 6.5cm] at (non-lin)
	...
	given $x + f'(p) \neq 0$
	$$\frac{dy}{dx} = x\frac{dp}{dx} + p + f(\frac{dp}{dx} \frac{dy}{dx})$$
	$$0 = \frac{dp}{dx} (x + f'(p))$$
	since $$x + f'(p) \neq 0, \frac{dp}{dx} = 0$$
...
\node[annotation, centered, font=, yshift = -7cm, xshift = -4.75cm, text width = 8cm] at (exact)
	...
	using (2),
	$$\frac{\partial}{\partial y}(\frac{\partial f}{\partial x}) = \frac{\partial M}{\partial y} = \frac{\partial}{\partial x}(\frac{\partial f}{\partial y}) = \frac{\partial N}{\partial x}$$
...
\node[annotation, centered, font=, yshift = 5cm, text width = 6.5cm] at (y)
	...
	$$\int \frac{d \mu}{\mu} = \int \frac{1}{M}(\frac{\partial N}{\partial x} - \frac{\partial M}{\partial y})dy$$
...
\node[annotation, centered, font=, yshift = -5cm, xshift = -5cm, text width = 8.5cm] at (2-homo)
	{$$a_0y + a_1(\frac{dy}{dx}) + a_2(\frac{d^2y}{dx^2}) + ... + a_n(\frac{d^ny}{dx^n})= 0$$
...
\node[annotation, centered, font=, xshift = -5cm, yshift = 4.5cm, text width = 8.5cm] at (2-non-homo)
	{$$a_0y + a_1(\frac{dy}{dx}) + a_2(\frac{d^2y}{dx^2}) + ... + a_n(\frac{d^ny}{dx^n}) = F(x) \neq 0$$
	$$y = y_c + y_p$$ 
...
\node[annotation, centered, font=, yshift = -4.5cm, text width = 9cm] at (trig)
	...
	$$a_0 + a_1m + a_2m^2 = 0, \rightarrow m = m_1, m_2$$
	if m is real
	$$assume \hspace{0.1cm} y_p = Asinx + Bcosx$$
	if m is complex and cos(bx) + sin(bx) are present in $y_c$
	$$assume \hspace{0.1cm} y_p = (Asin(bx) + Bcos(bx))x$$
	};
...
\node[annotation, centered, font=, xshift = 8cm, yshift = -12cm, text width = 11.5cm] at (LT)
	...
	$F(t) = Y'(t)$
	$$L\{Y'\} = \int_{0}^{\infty} e^{-st} Y'(t)$$
	$\hspace{1cm}$ integrate by parts
	$$\lim\limits_{M \rightarrow \infty} [e^{-st} Y(t) + s\int_{0}^{\infty} e^{-st} Y'(t)]$$
	$$\lim\limits_{M \rightarrow \infty} [e^{-sM} Y(M) - e^{-s(0)} Y(0) + s\int_{0}^{\infty} e^{-st} Y'(t)]$$
	...
	$$L^{-1}\{\frac{1}{s}\} = 1$$
	$$L^{-1}\{\frac{1}{s-a}\} = e^{at}$$
	$$L^{-1}\{\frac{s}{s^2 + \omega^2}\} = cos(\omega t)$$
	$$L^{-1}\{\frac{\omega}{s^2 + \omega^2}\} = sin(\omega t)$$
	};
